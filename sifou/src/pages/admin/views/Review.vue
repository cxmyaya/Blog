    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label='文章审核' name='article'>
        <el-table :data='blogList' size='mini'>
          ...
          <el-table-column align="center" prop='lastModified' label='发布时间' min-width='150' show-overflow-tooltip>
            <template slot-scope='scope'>
          ...
          <el-table-column align="center" label='tags' min-width='200'>
            <template slot-scope='scope'>
             <el-tag
              type='success'
          ...
          <el-table-column align="center" label='操作' min-width="150">
            <template slot-scope='scope'>
              <el-button type="success" size='mini' @click='approvedBlog(scope.row)'>通过</el-button>
      ...
      <el-tab-pane label='新用户审核' name='user'>
        <el-table :data="userList" size='mini'>
          ...
          <el-table-column label='操作' align="center" >
            <template slot-scope='scope'>
              <el-button type="success" size="mini" @click='approvedUser(scope.row)'>通过</el-button>
...
<script>
  ...
  export default {
    ...
    methods:{
      getUserList(){
        AuthorService.getUnapprovedUserList(this.searchUserParams).then(res => {
      ...
      getBlogList(){
        BlogService.getUnapprovedBlogList(this.searchBlogParams).then(res => {
          if(res.data.status === 200){
      ...
      approvedUser(row){
        AuthorService.approvedUser({key:row.key}).then(res => {
          if(res.data.status === 200){
      ...
      approvedBlog(row){
        BlogService.approvedBlog({blogId:row.blogId}).then(res => {
          if(res.data.status === 200){
            this.$message.success(res.data.message)
